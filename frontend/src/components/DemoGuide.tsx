...
export function DemoGuide({ demoViewRole, onStartTour, showTourButton }: DemoGuideProps) {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      <header>
        <h1 className="text-5xl font-black tracking-tight text-slate-900 dark:text-slate-100 mb-2">Demo Guide</h1>
        <p className="text-xl text-slate-700 dark:text-slate-400 font-medium">A quick tour of what Valley Science offers. Click around freely — this is your sandbox.</p>
      </header>
      ...
      <div className="bg-gradient-to-r from-soft-pink/20 to-sage-green/20 dark:from-soft-pink/10 dark:to-sage-green/10 p-8 rounded-[32px] border border-slate-100 dark:border-slate-800">
        <div className="flex items-start gap-4">
          <Sparkles className="text-soft-pink shrink-0 mt-1" />
          <div>
            <h3 className="font-black text-slate-900 dark:text-slate-100 mb-2">What we want to help kids with</h3>
...
function GuideCard({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return (
    <div className="bg-white dark:bg-slate-900 p-7 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-lg dark:shadow-black/20">
      <div className="w-11 h-11 bg-slate-50 dark:bg-slate-800 rounded-2xl flex items-center justify-center mb-4">{icon}</div>
      <h3 className="font-black text-slate-900 dark:text-slate-100 mb-2">{title}</h3>
